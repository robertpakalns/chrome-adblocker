const checkboxes = {
  ...
  scams: document.getElementById("scams"),
};
...
const loadSettings = async () => {
  ...
  const { enabledCategories = [] } =
    await chrome.storage.local.get("enabledCategories");
  for (const key in checkboxes)
    checkboxes[key].checked = enabledCategories.includes(key);
};
...
const saveSettings = async () => {
  const enabledCategories = Object.keys(checkboxes).filter(
